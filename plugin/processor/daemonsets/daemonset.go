...
func (m *Processor) exportCsv() []*golang.CSVRow {
	headers := []string{
		"Namespace", "Daemonset Name", "Pod Name", "Container Name",
	...
	for name, _ := range m.summary {
		...
		for _, pod := range daemonset.Pods {
			for _, container := range pod.Spec.Containers {
				...
				row = append(row, daemonset.Daemonset.Namespace, daemonset.Daemonset.Name, pod.Name, container.Name)

				var righSizing *golang2.KubernetesContainerRightsizingRecommendation
				if daemonset.Wastage != nil {
					for _, c := range daemonset.Wastage.Rightsizing.ContainerResizing {
