...
func NewProcessor(ctx context.Context, identification map[string]string, kubernetesProvider *kaytuKubernetes.Kubernetes, prometheusProvider *kaytuPrometheus.Prometheus, publishOptimizationItem func(item *golang.ChartOptimizationItem), publishResultSummary func(summary *golang.ResultSummary), jobQueue *sdk.JobQueue, configuration *kaytu.Configuration, client golang2.OptimizationClient, namespace *string, observabilityDays int) *Processor {
	r := &Processor{
		...
		items:                   util.NewMap[string, JobItem](),
...
func (m *Processor) ReEvaluate(id string, items []*golang.PreferenceItem) {
	...
	m.items.Set(id, v)
	m.jobQueue.Push(NewOptimizeJobJob(context.Background(), m, id))
...
func (m *Processor) exportCsv() []*golang.CSVRow {
	headers := []string{
		"Namespace", "Job Name", "Pod Name", "Container Name",
	...
	for name, _ := range m.summary {
		...
		for _, pod := range job.Pods {
			for _, container := range pod.Spec.Containers {
				...
				row = append(row, job.Job.Namespace, job.Job.Name, pod.Name, container.Name)

				var righSizing *golang2.KubernetesContainerRightsizingRecommendation
				if job.Wastage != nil {
					for _, c := range job.Wastage.Rightsizing.ContainerResizing {
...
func (m *Processor) UpdateSummary(itemId string) {
	...
	if ok && i.Wastage != nil {
		...
		if i.Job.Spec.Parallelism != nil {
			ds.ReplicaCount = *i.Job.Spec.Parallelism
		}
