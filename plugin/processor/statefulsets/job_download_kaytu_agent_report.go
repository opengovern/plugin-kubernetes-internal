...
func (j *DownloadKaytuAgentReportJob) Run(ctx context.Context) error {
	report, err := j.processor.kaytuClient.DownloadReport(ctx, "kubernetes-statefulsets")
	if err != nil {
		return err
	}
	...
	for _, i := range items {
		...
		if j.processor.selector != "" {
			if !shared.LabelFilter(j.processor.selector, item.Statefulset.Labels) {
				continue
			}
		}
